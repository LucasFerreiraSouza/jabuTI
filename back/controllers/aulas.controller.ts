  ...
  try {
    ...
    const conteudosFormatados = Array.isArray(conteudos)
      ? conteudos.map((c: any) => ({
          ...
          ordem: c.ordem ?? 0,
          backgroundColor: isValidHex(c.backgroundColor)
            ? c.backgroundColor
            : "#ffffff",
          textColor: isValidHex(c.textColor) ? c.textColor : "#000000",
          criadoPor,
          criadoPorUsername: ((req as any).user?.username || "").slice(0, 50)
        }))
      : [];
    ...
    const novaAula = await Aula.create({
      ...
      backgroundColor: isValidHex(backgroundColor)
        ? backgroundColor
        : "#ffffff",
      textColor: isValidHex(textColor) ? textColor : "#000000",
      criadoPor,
...
export const atualizarAula = async (req: Request, res: Response) => {
  try {
    ...
    if (backgroundColor !== undefined && isValidHex(backgroundColor)) {
      aula.backgroundColor = backgroundColor;
    }

    if (textColor !== undefined && isValidHex(textColor)) {
      aula.textColor = textColor;
    }
      ...
      conteudos.forEach((c: any) => {
        if (c._id) {
          ...
          if (conteudo) {
            ...
            conteudo.exercicio = Array.isArray(c.exercicio)
              ? c.exercicio
              : conteudo.exercicio;
            conteudo.ordem = c.ordem ?? conteudo.ordem;

            if (c.backgroundColor !== undefined) {
              conteudo.backgroundColor = isValidHex(c.backgroundColor)
                ? c.backgroundColor
                : conteudo.backgroundColor;
            }

            if (c.textColor !== undefined) {
              conteudo.textColor = isValidHex(c.textColor)
                ? c.textColor
                : conteudo.textColor;
            }
          }
...
export const deletarAula = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(200)
      .json({ mensagem: "Aula e imagens deletadas com sucesso" });
  } catch (err) {
    console.error(err);
