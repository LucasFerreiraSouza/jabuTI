...
describe("emailService", () => {
  ...
  it("deve enviar email com código 2FA", async () => {
    (emailTemplates.codigo2FA as jest.Mock).mockReturnValue({
      subject: "teste 2FA",
      html: "<p>codigo</p>",
    });

    await emailService.codigo2FA("email@test.com", "Lucas", "123456");
  ...
  it("deve enviar email de ativação de senha", async () => {
  ...
  it("deve enviar email de reset de email", async () => {
    (emailTemplates.resetEmail as jest.Mock).mockReturnValue({
      subject: "reset email",
      html: "<p>novo email</p>",
    });

    await emailService.resetEmail(
      "email@test.com",
      "Lucas",
      "link",
      "novo@email.com"
  ...
  it("deve enviar email de promovido admin", async () => {
  ...
  it("deve enviar email de despromovido admin", async () => {
